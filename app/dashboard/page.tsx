    ...
    <AppShell>
      <div className="p-6 md:p-10 max-w-5xl mx-auto animate-fade-in">
        ...
        <div className="mb-8 p-5 rounded-3xl bg-white shadow-apple animate-slide-up">
          <div className="flex items-center justify-between mb-3">
            ...
            {reportedDays >= totalDays ? (
              <span className="text-xs font-semibold px-3 py-1 rounded-full bg-green-100 text-green-700">הושלם ✓</span>
            ) : reportedDays > 0 ? (
              <span className="text-xs font-semibold px-3 py-1 rounded-full bg-amber-100 text-amber-700">בתהליך ⏳</span>
            ) : (
              <span className="text-xs font-semibold px-3 py-1 rounded-full bg-gray-100 text-gray-500">טרם התחיל</span>
            )}
          </div>
          ...
          <div className="flex justify-between mt-2">
            {["א'", "ב'", "ג'", "ד'", "ה'", "ו'"].map((label, i) => (
              <div key={i} className="flex flex-col items-center">
                <div className={cn("w-2 h-2 rounded-full mt-1", i < reportedDays ? "bg-green-400" : "bg-gray-200")} />
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
          {cards.map((card, i) => (
            <div key={card.title} className="animate-slide-up" style={{ animationDelay: `${i * 80}ms`, animationFillMode: "backwards" }}>
              {card.href !== "#" ? (
                <Link href={card.href} className="block group">
                  <div className={cn("h-[120px] rounded-3xl bg-gradient-to-br p-5 shadow-apple hover:shadow-apple-hover transition-apple cursor-pointer group-hover:scale-[1.02] duration-300 flex flex-col justify-between", card.gradient)}>
        ...
        <div className="rounded-3xl bg-white shadow-apple p-6 animate-slide-up" style={{ animationDelay: "400ms", animationFillMode: "backwards" }}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              <button key={action.label} onClick={action.onClick} className="flex items-center gap-3 p-3 rounded-2xl hover:bg-accent transition-apple text-right group">
                <div className={cn("w-9 h-9 rounded-xl flex items-center justify-center shrink-0", action.color)}>
                  <action.icon className="w-4 h-4" />
                </div>
                <span className="text-xs font-medium text-muted-foreground group-hover:text-foreground transition-apple">{action.label}</span>
              </button>
            ))}
          </div>
        </div>
